...
class _TurnoutScreenState extends State<TurnoutScreen> {
  ...
  List<Turnout> _filterSortAndSearchTurnouts(List<Turnout> turnouts) {
    ...
    return Scaffold(
            ...
            IconButton(
              ...
              onPressed: () {
                ...
                Future.delayed(Duration.zero, () {
                  ...
                  showSearch(
                    context: context,
                    delegate: _TurnoutSearchDelegate(
                      turnouts: Provider.of<TurnoutProvider>(context).turnouts,
                      now: _now,
      ...
      body: Consumer<TurnoutProvider>(
        builder: (context, provider, _) {
          ...
          final turnouts = _filterSortAndSearchTurnouts(provider.turnouts);
...
class _TurnoutSearchDelegate extends SearchDelegate {
  final List<Turnout> turnouts;
  final DateTime now;
  final String filterType;
  final String sortOrder;

  _TurnoutSearchDelegate({
    required this.turnouts,
    required this.now,
  ...
  Widget _buildSearchResults() {
    final results =
        turnouts.where((t) {
          ...
          final descMatch =
              ...
              false;
          final locMatch =
              t.location?.toLowerCase().contains(query.toLowerCase()) ?? false;
          if (!nameMatch && !descMatch && !locMatch) return false;

          // Apply date filter
          if (filterType == 'pending') return t.turnoutDate.isAfter(now);
          if (filterType == 'passed') return t.turnoutDate.isBefore(now);
          return true;
        }).toList()..sort(
          (a, b) => sortOrder == 'newest'
              ? b.turnoutDate.compareTo(a.turnoutDate)
              : a.turnoutDate.compareTo(b.turnoutDate),
        );
    ...
    if (results.isEmpty) {
      return Center(
        child: Text(
          query.isEmpty ? 'No events found' : 'No matching events for "$query"',
        ),
      );
    }
        ...
        return TurnoutCard(
          ...
          onTap: () {
            ...
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => TurnoutDetailsScreen(turnout: turnout),
              ),
            );
          },
        );
